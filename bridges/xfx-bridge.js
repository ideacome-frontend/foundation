...
function init(p, sys) {
    ...
    handler = handlers[p];
    xfxBridge.respond = respond;
    xfxBridge.sendEvent = sendEvent;
    xfxBridge.fetchParams = fetchParams;
...
var _paramstrq = [];
var _callback = {};
window.xfxBridgeParam = function () {
    return _paramstrq.shift();
};
window.xfxBridgeRespond = function (dict) {
    setTimeout(function () {
...
function _methodFor(cap, fallback) {
    function _route() {
        if (!_supports(cap)) {
            if (fallback) {
