 ...
 * 家庭自用车 true
 */
export function isNotCompanyCar(name: string): boolean {
    let noInputAttr = ['公司', '集团', '基金', '合作社', '团体', '协会', '银行', '政府', '局', '事务所', '法院', '医院', '大学', '中学', '小学', '幼儿园', '街道', '社区', '办事处', '厂'];
    let flag        = true;
    if (name && name.length > 4) {
        for (let item of noInputAttr) {
