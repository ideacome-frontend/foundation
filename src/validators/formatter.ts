 ...
 */
export function wxbFromString(str: string): any {
...
/*格式化时间戳*/
export function formatTime(addTime: any) {
    let weekDay = '';
    let today = new Date();
    today.setHours(0);
    today.setMinutes(0);
 ...
 * @param telNo 手机号
 */
export function formatTelNum(telNo: any) {
    let inputPhone = '';
 ...
 * @param n, 小数点位数
 */
export function formatMoney(money: any, n: any) {
    n = n > 0 && n <= 20 ? n : 2;
 ...
 * @return {string}
 */
export function parseJsonToString(qObj: any): string {
    let qArr: any = [];
    if (qObj) {
        for (let i in qObj) {
            qArr.push(i + '=' + qObj[i]);
 ...
 */
export function moneyCuter(s: any): string {
    if (/[^0-9\.]/.test(s)) {
        return "0.00";
    }
    if (s == null || s == "") {
 ...
 * */
export function cutNumberByDigits(num: any, digits = 2): any {
